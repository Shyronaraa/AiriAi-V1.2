...
conn.sendEvent = async (jid, text, des, loc, link) => {
    let msg = generateWAMessageFromContent(jid, {
        messageContextInfo: {
            messageSecret: crypto.randomBytes(32).toString('hex'),
        },
        eventMessage: {
            isCanceled: false,
            name: text,
...
exports.smsg = (conn, m, hasParent) => {
  ...
  if (m.key) {
    ...
    m.isBaileys =
      (m.id && m.id.length === 16) ||
      (m.id.startsWith("AIRII") && m.id.length === 12) ||
      false;
...
const a = {
        contextInfo: {
            ...
            forwardedNewsletterMessageInfo: {
                newsletterJid: '120363278878512006@newsletter',
                newsletterName: "⧉ᴄᴏᴅᴇɴᴀᴍᴇ: ᴀɪʀɪɪ ᴠ1.1 ʙᴇᴛᴀ",
                serverMessageId: -1
            },
        }
    }
 ...
 if (options && pesan) {
  ...
  m.reply = async (pesan, options) => {
    ...
    try {
      if (options && pesan) {
        conn.sendFile(m.chat, options, null, pesan, fkontak, null, a);
      } else {
        if (pesan) {
          conn.reply(m.chat, pesan, fkontak, a);
        } else {
          conn.reply(m.chat, options, fkontak, a);
        }
      }
    } catch (e) {
      conn.reply(m.chat, pesan, fkontak, a);
